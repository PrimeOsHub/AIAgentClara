...
async function createStdioClientContext(client, definition, logger) {
    const resolvedEnvOverrides = definition.env && Object.keys(definition.env).length > 0
        ? Object.fromEntries(Object.entries(definition.env)
...
async function connectSseFallbackTransport(client, definition, command, transportOptions, oauthSession, logger, options) {
    try {
        const transport = await connectHttpTransport(client, new SSEClientTransport(command.url, transportOptions), oauthSession, logger, {
            serverName: definition.name,
            serverUrl: command.url,
            maxAttempts: options.maxOAuthAttempts,
            oauthTimeoutMs: options.oauthTimeoutMs,
...
export async function createClientContext(definition, logger, clientInfo, options = {}) {
    ...
    return withEnvOverrides(activeDefinition.env, async () => {
        if (activeDefinition.command.kind === 'stdio') {
            return createStdioClientContext(client, activeDefinition, logger);
        }
