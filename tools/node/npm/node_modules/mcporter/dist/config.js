...
import { listConfigLayerPaths as discoverConfigLayerPaths, resolveConfigPath as discoverConfigPath, } from './config/path-discovery.js';
...
import { DEFAULT_IMPORTS, RawEntrySchema, } from './config-schema.js';
...
export async function loadDaemonConfig(options = {}) {
    const rootDir = options.rootDir ?? process.cwd();
    const layers = await loadConfigLayers(options, rootDir);
    let idleTimeoutMs;
    for (const layer of layers) {
