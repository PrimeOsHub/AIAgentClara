...
export async function handleImportCommand(options, args) {
    ...
    const paths = flags.path ? [path.resolve(expandHome(flags.path))] : pathsForImport(kind, rootDir);
    const entries = [];
    const seenNames = new Set();
    for (const candidate of paths) {
        const resolved = expandHome(candidate);
        const map = await readExternalEntries(resolved, { projectRoot: rootDir, importKind: kind });
        if (!map) {
            continue;
        }
...
function resolveImportCopyTarget(configPath, rootDir) {
...
function extractImportFlags(args) {
