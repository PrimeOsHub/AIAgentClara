...
import { buildAuthCommandHint, buildJsonListEntry, createEmptyStatusCounts, createUnknownResult, printBriefTool, printSingleServerHeader, printToolDetail, summarizeStatusCounts, } from './list-output.js';
...
export async function handleList(runtime, args) {
    ...
    if (!target) {
        ...
        try {
            ...
            if (!flags.quiet && flags.format === 'text') {
                console.log(`mcporter ${MCPORTER_VERSION} — Listing ${servers.length} server(s) (per-server timeout: ${perServerTimeoutSeconds}s)`);
            }
            const spinner = !flags.quiet && flags.format === 'text' && supportsSpinner
                ? ora(`Discovering ${servers.length} server(s)…`).start()
                : undefined;
            const renderedResults = !flags.quiet && flags.format === 'text'
                ? Array.from({ length: servers.length }, () => undefined)
                : undefined;
            const summaryResults = Array.from({ length: servers.length }, () => undefined);
            let completedCount = 0;
            const tasks = servers.map((server, index) => checkListServer(runtime, server, perServerTimeoutMs).then((result) => {
    ...
    const sourcePath = definition.sources?.length || definition.source
    ...
    if (flags.statusOnly) {
        ...
        try {
            ...
            if (flags.format === 'json') {
                console.log(JSON.stringify({ mode: 'list', counts: summarizeStatusCounts([entry]), servers: [entry] }, null, 2));
                return;
            }
            const rendered = renderServerListRow(result, timeoutMs, { verbose: flags.verbose });
            console.log(rendered.line);
            console.log(`✔ Listed 1 server (${entry.status === 'ok' ? '1 healthy' : `0 healthy; 1 ${statusLabel(entry.status)}`}).`);
            return;
        }
        finally {
    ...
    try {
        if (flags.format === 'json') {
            try {
                const metadataEntries = filterToolMetadata(await withTimeout(loadToolMetadata(runtime, target, {
                    includeSchema: true,
                    autoAuthorize: false,
                    allowCachedAuth: true,
                }), timeoutMs), requestedTool);
                await persistPreparedEphemeralServer(runtime, prepared);
...
async function checkListServer(runtime, server, timeoutMs) {
    const startedAt = Date.now();
    try {
        const tools = await withTimeout(runtime.listTools(server.name, { autoAuthorize: false, allowCachedAuth: true }), timeoutMs);
        return {
            server,
            status: 'ok',
            tools,
...
function maybeSetListExitCode(entries, flags) {
    if (!flags.exitCode) {
        return;
    }
...
function resolveServerDefinition(runtime, name, options = {}) {
    try {
...
function suggestServerName(runtime, attempted) {
...
function resolveConfiguredToolSelector(runtime, target) {
...
function filterToolMetadata(entries, requestedTool) {
    if (!requestedTool) {
        return entries;
    }
    return entries.filter((entry) => entry.tool.name === requestedTool);
}
function printMissingToolText(definition, tool, durationMs, transportSummary, sourcePath) {
...
function printMissingToolJson(definition, tool, durationMs, transportSummary, flags) {
...
async function loadServerInstructions(runtime, serverName) {
