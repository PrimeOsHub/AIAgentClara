...
function buildCommandSections(colorize) {
    const sections = [
        {
            title: 'Core commands',
            entries: [
...
function formatCommandSection(section, colorize) {
...
function formatGlobalFlags(colorize) {
...
function formatQuickStart(colorize) {
...
function formatHelpFooter(colorize) {
    const pointer = 'Run `mcporter <command> --help` for detailed flags.';
    const autoLoad = 'mcporter auto-loads servers from ./config/mcporter.json and editor imports (Cursor, Claude, Codex, etc.).';
    if (!colorize) {
        return `${pointer}\n${autoLoad}`;
    }
...
export function consumeHelpTokens(args) {
    let found = false;
