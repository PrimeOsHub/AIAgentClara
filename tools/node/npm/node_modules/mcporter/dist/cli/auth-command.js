...
export async function handleAuth(runtime, args) {
    ...
    const ephemeralSpec = extractEphemeralServerFlags(args);
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
        try {
            ...
            const tools = await withInfoLogsSuppressed(noBrowser, () => runtime.listTools(target, {
                autoAuthorize: true,
                ...(noBrowser
                    ? {
...
async function withInfoLogsSuppressed(enabled, task) {
    if (!enabled) {
        return task();
    }
...
async function runStdioAuth(definition, options = {}) {
...
function buildNoBrowserOAuthOptions(format, markAuthorizationOutputEmitted) {
    return {
        suppressBrowserLaunch: true,
        onAuthorizationUrl(request) {
...
function consumeBrowserSuppression(args, env) {
...
function resolveBrowserSuppressionFromEnv(raw) {
    if (raw === undefined) {
        return 'default';
    }
