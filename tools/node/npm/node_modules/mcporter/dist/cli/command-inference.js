...
export function inferCommandRouting(token, args, definitions) {
    if (!token) {
...
function isCallLikeToken(token) {
    if (!token) {
        return false;
    }
...
function isExplicitCommand(token) {
    return (token === 'list' ||
        token === 'call' ||
        token === 'auth' ||
        token === 'vault' ||
...
function isHttpToolToken(token) {
    if (!token) {
        return false;
    }
    return splitHttpToolSelector(token) !== null;
}
//# sourceMappingURL=command-inference.js.map