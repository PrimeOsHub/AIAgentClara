...
const BUNDLED_DEPENDENCIES = ['commander', 'mcporter', 'jsonc-parser'];
...
export async function bundleOutput({ sourcePath, targetPath, runtimeKind, minify, bundler, }) {
    if (bundler === 'bun') {
...
async function bundleWithRolldown({ sourcePath, targetPath, runtimeKind, minify, }) {
    let rolldownImpl;
    try {
        ({ rolldown: rolldownImpl } = await import('rolldown'));
    }
    catch (error) {
        const message = 'Rolldown bundling is unavailable in this build of mcporter; rerun with --bundler bun or install mcporter via npm (Node.js) to use the Rolldown bundler.';
    ...
    const bundle = await rolldownImpl({
        ...
        onLog(level, log, handler) {
            if (typeof log.code === 'string' && log.code === 'EVAL') {
                return;
            }
...
function isExpectedNodeBuiltinWarning(log) {
    const record = log;
...
async function bundleWithBun({ sourcePath, targetPath, runtimeKind, minify, }) {
    const absTarget = path.resolve(targetPath);
    await fs.mkdir(path.dirname(absTarget), { recursive: true });
...
export async function compileBundleWithBun(bundlePath, outputPath) {
    const bunBin = await verifyBunAvailable();
    await new Promise((resolve, reject) => {
        execFile(bunBin, ['build', bundlePath, '--compile', '--outfile', outputPath], { cwd: process.cwd(), env: process.env }, (error) => {
            if (error) {
                reject(error);
                return;
            }
...
export function resolveBundleTarget({ bundle, compile, outputPath, }) {
...
export function computeCompileTarget(compileOption, _bundlePath, serverName) {
...
function createLocalDependencyAliasPlugin(specifiers) {
...
function resolveLocalDependency(specifier) {
    try {
...
async function ensureBundlerDeps(stagingDir) {
    const nodeModulesDir = path.join(stagingDir, 'node_modules');
    await fs.mkdir(nodeModulesDir, { recursive: true });
    await Promise.all(BUNDLED_DEPENDENCIES.map(async (specifier) => {
...
async function findMissingBundlerDeps(stagingDir) {
    const missing = [];
...
async function installPublishedBundlerDeps(stagingDir) {
    const installSpec = process.env.MCPORTER_BUNDLER_DEP_PACKAGE ?? MCPORTER_VERSION;
    if (installSpec === '0.0.0-dev') {
        throw new Error('Unable to resolve generated-CLI bundler dependencies from this standalone mcporter binary. Install mcporter via npm/Homebrew or publish the matching mcporter package before using --compile.');
    }
    await fs.writeFile(path.join(stagingDir, 'package.json'), JSON.stringify({ private: true, type: 'module', dependencies: { mcporter: installSpec } }, null, 2), 'utf8');
    await new Promise((resolve, reject) => {
        execFile('npm', ['install', '--ignore-scripts', '--no-audit', '--no-fund', '--min-release-age=0'], { cwd: stagingDir, env: process.env }, (error) => {
            if (error) {
                reject(new Error(`Unable to install ${formatMcporterInstallSpec(installSpec)} dependencies needed for Bun compilation from this standalone binary. Install mcporter via npm/Homebrew, or ensure npm can reach the registry.\n\n${error.message}`));
                return;
            }
            resolve();
        });
...
async function linkOrCopyDependency(sourceDir, targetDir) {
    try {
...
function resolveUniquePath(directory, baseName) {
