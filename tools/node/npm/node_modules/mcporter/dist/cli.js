...
export async function printAuthHelp() {
...
export async function handleGenerateCli(...args) {
    const { handleGenerateCli: imported } = await import('./cli/generate-cli-runner.js');
    return imported(...args);
}
export async function handleInspectCli(...args) {
...
export async function handleResource(...args) {
...
async function maybeHandleDaemonFastCall(command, args, configResolution, rootDir) {
...
async function maybeHandleSimpleDaemonFastCall(callArgs, configResolution, rootDir) {
...
function resolveDaemonFastCallArgs(command, args) {
    if (command === 'call') {
        return args;
    }
...
function isExplicitNonCallCommand(command) {
    return (command === 'list' ||
        command === 'auth' ||
        command === 'resource' ||
        command === 'resources' ||
...
function resolveExplicitCallServer(args) {
    let serverFlag;
...
function createDaemonOnlyRuntime(daemonClient) {
    return {
        ...
        listTools: async (server, options) => (await daemonClient.listTools({
            server,
            includeSchema: options?.includeSchema,
            autoAuthorize: options?.autoAuthorize,
        })),
        callTool: (server, toolName, options) => daemonClient.callTool({
            server,
            tool: toolName,
            args: options?.args,
