...
<head>
  ...
  <title>mcporter — MCP, made portable.</title>
  <meta name="description" content="mcporter is a TypeScript runtime, CLI, and code-generation toolkit for the Model Context Protocol — built so AI agents and developers can call any MCP server without boilerplate.">
  <link rel="canonical" href="https://mcporter.sh/">
  <meta property="og:type" content="website">
  <meta property="og:site_name" content="mcporter">
  <meta property="og:title" content="mcporter — MCP, made portable.">
  <meta property="og:description" content="mcporter is a TypeScript runtime, CLI, and code-generation toolkit for the Model Context Protocol — built so AI agents and developers can call any MCP server without boilerplate.">
  <meta property="og:url" content="https://mcporter.sh/">
  <meta property="og:image" content="https://mcporter.sh/social-card.png">
  <meta property="og:image:width" content="1200">
  <meta property="og:image:height" content="630">
  <meta name="twitter:card" content="summary_large_image">
  <meta name="twitter:title" content="mcporter — MCP, made portable.">
  <meta name="twitter:description" content="mcporter is a TypeScript runtime, CLI, and code-generation toolkit for the Model Context Protocol — built so AI agents and developers can call any MCP server without boilerplate.">
...
npx mcporter emit-ts linear --mode client --out src/linear-client.ts</code></pre>
<p><code>--json</code> produces a stable JSON envelope on stdout; human progress, prompts, and warnings always go to stderr so pipes stay parseable.</p>
<h2 id="what-mcporter-does"><a class="anchor" href="#what-mcporter-does" aria-label="Anchor link">#</a>What mcporter does</h2>
<p>mcporter leans into the <strong>code-execution-with-MCP</strong> pattern Anthropic recommends: skip the giant tool-schema prompt, generate a small typed surface, and let the agent or the human call MCP servers like normal functions.</p>
<ul>
<li><strong>Zero-config discovery.</strong> Reads your home config (<code>~/.mcporter/mcporter.json[c]</code>, or <code>$XDG_CONFIG_HOME/mcporter/mcporter.json[c]</code>), then <code>config/mcporter.json</code>, then imports from Cursor / Claude / Codex / Windsurf / OpenCode / VS Code. <code>${ENV}</code> placeholders are expanded; transports are pooled across calls.</li>
<li><strong>One-command CLI generation.</strong> <a href="cli-generator.html"><code>mcporter generate-cli</code></a> turns any MCP server into a ready-to-run CLI with embedded schemas, optional Rolldown/Bun bundling, and Bun-compiled binaries.</li>
<li><strong>Typed clients.</strong> <a href="emit-ts.html"><code>mcporter emit-ts</code></a> emits <code>.d.ts</code> interfaces or a ready-to-run client wrapping <code>createServerProxy()</code> so agents call MCP tools with full TypeScript types.</li>
<li><strong>Friendly composable API.</strong> <a href="tool-calling.html"><code>createServerProxy()</code></a> maps tools to camelCase methods, applies JSON-schema defaults, validates required arguments, and returns a <code>CallResult</code> with <code>.text()</code>, <code>.markdown()</code>, <code>.json()</code>, <code>.images()</code>, <code>.content()</code> helpers.</li>
<li><strong>Ad-hoc connections + auto-OAuth.</strong> Point the CLI at any MCP endpoint (HTTP, SSE, stdio) without touching config. Hosted MCPs that need a browser login (Supabase, Vercel, etc.) are auto-detected — <code>mcporter auth &lt;url&gt;</code> promotes the definition to OAuth on the fly. See <a href="adhoc.html">Ad-hoc connections</a>.</li>
<li><strong>MCP bridge for agents.</strong> <code>mcporter serve --stdio</code> exposes daemon-managed keep-alive servers as one MCP server, with tools namespaced as <code>server__tool</code>, so clients can share the same warm daemon-backed transports.</li>
<li><strong>OAuth &amp; stdio ergonomics.</strong> Built-in OAuth caching, token refresh, log tailing, and stdio wrappers — same interface across HTTP, SSE, and stdio transports.</li>
</ul>
<h2 id="built-for-agents"><a class="anchor" href="#built-for-agents" aria-label="Anchor link">#</a>Built for agents</h2>
<p>mcporter is designed to be the layer between an MCP server and a coding agent. The pattern we recommend:</p>
<ol>
<li>Configure the server once (or import from your editor of choice).</li>
<li>Run <a href="emit-ts.html"><code>mcporter emit-ts &lt;server&gt;</code></a> to get a <code>.d.ts</code> of the tool surface.</li>
<li>Wire small per-server <a href="agent-skills.html">agent skills</a> instead of one mega-schema prompt — small prompts, named tools, no unrelated schemas loaded.</li>
<li>For shareable workflows, generate a standalone CLI with <a href="cli-generator.html"><code>mcporter generate-cli</code></a>.</li>
</ol>
<p>Because every transport flows through the same runtime, an agent that knows how to spawn <code>mcporter call</code> works with stdio servers, hosted HTTP MCPs, OAuth-gated services, and one-off URLs alike.</p>
<h2 id="why-a-porter"><a class="anchor" href="#why-a-porter" aria-label="Anchor link">#</a>Why a porter?</h2>
<p>A <em>porter</em> carries luggage between trains. mcporter does the same for MCP servers: it carries tool calls, schemas, OAuth tokens, and stdio handles between your agent (or your terminal) and whichever MCP server happens to be at the other end of the line. You don&#39;t have to know the shape of the server ahead of time, and the runtime keeps the connection warm so repeat calls are cheap.</p>
<h2 id="where-to-next"><a class="anchor" href="#where-to-next" aria-label="Anchor link">#</a>Where to next</h2>
<ul>
<li><a href="install.html">Install</a> — npm, npx, Homebrew, or the standalone Bun-compiled binary.</li>
<li><a href="quickstart.html">Quickstart</a> — your first list/call/resource in five minutes.</li>
<li><a href="config.html">Configuration</a> — <code>mcporter.json</code>, imports, env interpolation, OAuth.</li>
<li><a href="cli-reference.html">CLI reference</a> — every subcommand and flag.</li>
<li><a href="adhoc.html">Ad-hoc connections</a> — point at any MCP endpoint without editing config.</li>
<li><a href="agent-skills.html">Agent skills</a> — exposing servers to agents the right way.</li>
</ul></article>
