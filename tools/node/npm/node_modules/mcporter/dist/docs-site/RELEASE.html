...
<head>
  ...
  <title>Release Checklist — mcporter</title>
  <meta name="description" content="Release Checklist — mcporter documentation.">
  <link rel="canonical" href="https://mcporter.sh/RELEASE.html">
  <meta property="og:type" content="website">
  <meta property="og:site_name" content="mcporter">
  <meta property="og:title" content="Release Checklist — mcporter">
  <meta property="og:description" content="Release Checklist — mcporter documentation.">
  <meta property="og:url" content="https://mcporter.sh/RELEASE.html">
  <meta property="og:image" content="https://mcporter.sh/social-card.png">
  <meta property="og:image:width" content="1200">
  <meta property="og:image:height" content="630">
  <meta name="twitter:card" content="summary_large_image">
  <meta name="twitter:title" content="Release Checklist — mcporter">
  <meta name="twitter:description" content="Release Checklist — mcporter documentation.">
    ...
    <main>
      <header class="hero">
        ...
        <div class="hero-meta">
          ...
          <a class="edit" href="https://github.com/openclaw/mcporter/edit/main/docs/RELEASE.md" rel="noopener">Edit page</a>
        </div>
      </header>
      <div class="doc-grid">
        <article class="doc"><h1 id="release-checklist">Release Checklist</h1>
<blockquote><p><strong>Runner note:</strong> From the repo root run <code>export MCP_RUNNER=&quot;$PWD/runner&quot;</code> and use <code>$MCP_RUNNER &lt;command&gt;</code> for every shell command listed below unless the step explicitly says otherwise. This keeps the guardrails active even when the checklist jumps between directories.</p></blockquote>
<blockquote><p><strong>Helper script:</strong> You can run <code>./scripts/release.sh &lt;phase&gt;</code> (gates | artifacts | publish | smoke | tag | all) to execute the steps below with the runner by default. It stops on first error; rerun the next phase after fixing issues.</p></blockquote>
<blockquote><p><strong>No-warning policy:</strong> Every command below must finish without warnings (Oxfmt, Oxlint, tsgo, Vitest, npm pack, etc.). Fix issues before continuing; releases cannot ship with outstanding warnings.</p></blockquote>
<h2 id="definition-of-released"><a class="anchor" href="#definition-of-released" aria-label="Anchor link">#</a>Definition of “released”</h2>
<p>Shipping a release means <strong>all</strong> of:</p>
<ul>
<li>Tag pushed (<code>v&lt;version&gt;</code>).</li>
<li>npm published (<code>mcporter@&lt;version&gt;</code> visible via <code>npm view mcporter version</code>).</li>
<li>GitHub release published for the tag <strong>with assets + checksums</strong>.</li>
<li>Homebrew tap updated (and verified) after assets propagate.</li>
</ul>
<ol>
<li>Update version in package.json and src/runtime.ts.</li>
<li>Run pnpm install to refresh the lockfile if dependencies changed.</li>
<li>pnpm check (zero warnings allowed; abort immediately on any error)</li>
<li>pnpm test (must finish with <strong>0 failed</strong>; if Vitest prints any red FAIL lines or a non-zero exit code, stop and fix it before proceeding)</li>
<li>pnpm build</li>
<li>pnpm build:bun</li>
<li>tar -C dist-bun -czf dist-bun/mcporter-macos-arm64-v&lt;version&gt;.tar.gz mcporter</li>
<li>shasum -a 256 dist-bun/mcporter-macos-arm64-v&lt;version&gt;.tar.gz | tee dist-bun/mcporter-macos-arm64-v&lt;version&gt;.tar.gz.sha256</li>
<li>npm pack --pack-destination /tmp &amp;&amp; mv /tmp/mcporter-&lt;version&gt;.tgz . # keep the real tarball</li>
<li>shasum mcporter-&lt;version&gt;.tgz &gt; mcporter-&lt;version&gt;.tgz.sha1 &amp;&amp; shasum -a 256 mcporter-&lt;version&gt;.tgz &gt; mcporter-&lt;version&gt;.tgz.sha256</li>
<li>Verify git status is clean.</li>
<li>git commit &amp;&amp; git push.</li>
<li>pnpm publish --tag latest <em>(the runner already has npm credentials configured, so you can run this directly in the release shell; bump <code>timeout_ms</code> if needed because prepublish re-runs check/test/build and can take several minutes.)</em></li>
<li><code>npm view mcporter version</code> (and <code>npm view mcporter time</code>) to ensure the registry reflects the new release before proceeding. If the new version isn’t visible yet, wait a minute and retry—npm’s replication can lag briefly.</li>
<li>Sanity-check the “one weird trick” workflow from a <strong>completely empty</strong> directory (no package.json/node_modules) via:</li>
<p>``<code>bash rm -rf /tmp/mcporter-empty &amp;&amp; mkdir -p /tmp/mcporter-empty cd /tmp/mcporter-empty # run this without the runner because we are outside the repo and npx handles its own logging npx mcporter@&lt;version&gt; generate-cli &quot;npx -y chrome-devtools-mcp&quot; --compile ./chrome-devtools-mcp --help | head -n 5 </code>`` Only continue once the CLI compiles and the help banner prints.</p>
<li>Draft the GitHub release notes using this template (copy/paste and edit). <strong>Title the release <code>mcporter v&lt;version&gt;</code> (project name + version) to keep GitHub’s releases list consistent.</strong></li>
</ol>
<p>```markdown ## Highlights</p>
<ul>
<li>&lt;top feature&gt;</li>
<li>&lt;second feature&gt;</li>
<li>&lt;bugfix or UX callout&gt;</li>
</ul>
<p>SHA256 (mcporter-macos-arm64-v&lt;version&gt;.tar.gz): <code>&lt;sha from step 8&gt;</code> SHA256 (mcporter-&lt;version&gt;.tgz): <code>&lt;sha from npm pack&gt;</code> ```</p>
<p>Then <strong>create the GitHub release for tag v&lt;version&gt;</strong> and upload all assets:</p>
<ul>
<li><code>mcporter-macos-arm64-v&lt;version&gt;.tar.gz</code></li>
<li><code>mcporter-macos-arm64-v&lt;version&gt;.tar.gz.sha256</code> (from step 8; add a <code>.sha256</code> file)</li>
<li><code>mcporter-&lt;version&gt;.tgz</code> (from <code>npm pack</code>)</li>
<li><code>mcporter-&lt;version&gt;.tgz.sha1</code> and <code>mcporter-&lt;version&gt;.tgz.sha256</code></li>
<p>Double-check the uploaded checksums match your local files.</p>
</ul>
<ol>
<li>Tag the release (git tag v&lt;version&gt; &amp;&amp; git push --tags).</li>
<li>Post-tag housekeeping: add a fresh &quot;Unreleased&quot; stub to CHANGELOG.md (set to &quot;- Nothing yet.&quot;) and start a new version section for the just-released patch if it isn’t already recorded.</li>
</ol>
<p>After the release is live, always update the Homebrew tap and re-verify both installers. The tap formula should install the npm <code>.tgz</code>, not the Bun-compiled macOS tarball, because <code>generate-cli --compile</code> needs the installed package tree so Bun can resolve <code>mcporter</code>, <code>commander</code>, and related dependencies when compiling from an empty directory. Keep the macOS tarball on the GitHub release as a direct binary asset, but point Homebrew at <code>mcporter-&lt;version&gt;.tgz</code>.</p>
<ol>
<li>Update <code>steipete/homebrew-tap</code> -&gt; <code>Formula/mcporter.rb</code> with:</li>
</ol>
<ul>
<li>URL <code>https://github.com/steipete/mcporter/releases/download/v&lt;version&gt;/mcporter-&lt;version&gt;.tgz</code></li>
<li>SHA256 from <code>mcporter-&lt;version&gt;.tgz.sha256</code></li>
<li><code>require &quot;language/node&quot;</code>, <code>depends_on &quot;node&quot;</code>, and <code>system &quot;npm&quot;, &quot;install&quot;, *std_npm_args, &quot;--min-release-age=0&quot;</code> so same-day releases with fresh npm dependencies can install immediately.</li>
<p>Refresh the tap README highlight so Homebrew users see the new version callout.</p>
</ul>
<ol>
<li>Commit and push the tap update.</li>
<li>Refresh and reinstall from the real tap:</li>
<p>``<code>bash brew update brew reinstall steipete/tap/mcporter brew test steipete/tap/mcporter /opt/homebrew/bin/mcporter --version </code>``</p>
<li>Run a Homebrew-installed empty-directory compile smoke:</li>
<p>``<code>bash rm -rf /tmp/mcporter-brew-smoke &amp;&amp; mkdir -p /tmp/mcporter-brew-smoke cd /tmp/mcporter-brew-smoke /opt/homebrew/bin/mcporter generate-cli &quot;npx -y chrome-devtools-mcp&quot; --compile ./chrome-devtools-mcp --help | head -n 5 </code>``</p>
<li>Install the npm package globally (or leave it to npx) and verify that path too:</li>
<p>``<code>bash npm install -g mcporter@&lt;version&gt; mcporter --version npx --yes mcporter@&lt;version&gt; --version </code>``</p>
<li>If installing on another Mac, repeat the real tap reinstall and compile smoke there:</li>
<p>``<code>bash brew update brew reinstall steipete/tap/mcporter /opt/homebrew/bin/mcporter --version </code>``</p>
</ol><nav class="page-nav" aria-label="Pager"><a class="page-nav-prev" href="subagent.html"><small>Previous</small><span>Claude Subagent Quickstart</span></a><a class="page-nav-next" href="manual-testing.html"><small>Next</small><span>Manual Testing Harness</span></a></nav></article>
