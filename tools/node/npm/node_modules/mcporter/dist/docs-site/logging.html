...
<head>
  ...
  <title>Logging &amp; Diagnostics — mcporter</title>
  <meta name="description" content="Logging &amp; Diagnostics — mcporter documentation.">
  <link rel="canonical" href="https://mcporter.sh/logging.html">
  <meta property="og:type" content="website">
  <meta property="og:site_name" content="mcporter">
  <meta property="og:title" content="Logging &amp; Diagnostics — mcporter">
  <meta property="og:description" content="Logging &amp; Diagnostics — mcporter documentation.">
  <meta property="og:url" content="https://mcporter.sh/logging.html">
  <meta property="og:image" content="https://mcporter.sh/social-card.png">
  <meta property="og:image:width" content="1200">
  <meta property="og:image:height" content="630">
  <meta name="twitter:card" content="summary_large_image">
  <meta name="twitter:title" content="Logging &amp; Diagnostics — mcporter">
  <meta name="twitter:description" content="Logging &amp; Diagnostics — mcporter documentation.">
    ...
    <main>
      <header class="hero">
        ...
        <div class="hero-meta">
          ...
          <a class="edit" href="https://github.com/openclaw/mcporter/edit/main/docs/logging.md" rel="noopener">Edit page</a>
        </div>
      </header>
      <div class="doc-grid">
        <article class="doc"><h1 id="logging-diagnostics">Logging &amp; Diagnostics</h1>
<h2 id="daemon-logging"><a class="anchor" href="#daemon-logging" aria-label="Anchor link">#</a>Daemon logging</h2>
<p>The keep-alive daemon can tee its stdout/stderr (and per-server call traces) into a file so you can see crashes or repeated failures without rerunning it in the foreground.</p>
<h3 id="cli-flags"><a class="anchor" href="#cli-flags" aria-label="Anchor link">#</a>CLI flags</h3>
<ul>
<li><code>mcporter daemon start --log</code> — enable logging at the default path <code>~/.mcporter/daemon/daemon-&lt;config-hash&gt;.log</code>, or <code>$XDG_STATE_HOME/mcporter/daemon/daemon-&lt;config-hash&gt;.log</code> when <code>XDG_STATE_HOME</code> is set.</li>
<li><code>mcporter daemon start --log-file /tmp/mcporter-daemon.log</code> — write logs to a specific file (path is created if needed).</li>
<li><code>mcporter daemon start --log-servers chrome-devtools,mobile-mcp</code> — only emit per-call entries for the listed servers. Without this flag, <code>--log</code> records every keep-alive server’s calls.</li>
</ul>
<p><code>mcporter daemon restart</code> accepts the same logging flags, so you can bounce the daemon without retyping your tracing preferences.</p>
<p>Foreground mode (<code>mcporter daemon start --foreground --log</code>) still prints to your terminal; the log file mirrors the same content.</p>
<h3 id="environment-overrides"><a class="anchor" href="#environment-overrides" aria-label="Anchor link">#</a>Environment overrides</h3>
<p>Set these before invoking <code>mcporter</code> (helpful for scripts/CI):</p>
<ul>
<li><code>MCPORTER_DAEMON_LOG=1</code> — enable logging.</li>
<li><code>MCPORTER_DAEMON_LOG_PATH=/tmp/mcporter-daemon.log</code> — custom log file.</li>
<li><code>MCPORTER_DAEMON_LOG_SERVERS=chrome-devtools,playwright</code> — per-server call filtering.</li>
</ul>
<p>The CLI flags take precedence; env vars are the fallback.</p>
<h3 id="per-server-opt-in"><a class="anchor" href="#per-server-opt-in" aria-label="Anchor link">#</a>Per-server opt-in</h3>
<p>Add a logging block inside the server definition (alongside <code>lifecycle</code>) when you only want a specific STDIO transport to produce call traces, regardless of global flags:</p>
...
<p>When combined with <code>--log</code>/<code>MCPORTER_DAEMON_LOG=1</code>, any server that has <code>logging.daemon.enabled</code> emits <code>callTool start/success/error</code> lines even if it isn’t listed in <code>--log-servers</code>.</p>
<h3 id="inspecting-logs"><a class="anchor" href="#inspecting-logs" aria-label="Anchor link">#</a>Inspecting logs</h3>
<ul>
<li><code>mcporter daemon status</code> prints both the socket path and the current log file (if logging is enabled) so you can <code>tail -f</code> the output quickly.</li>
<li>Each call looks like:</li>
<p>``<code> [daemon] 2025-11-10T15:08:21.123Z callTool start server=chrome-devtools tool=take_snapshot [daemon] 2025-11-10T15:08:22.004Z callTool success server=chrome-devtools tool=take_snapshot </code>``</p>
<li>Shutdown events and errors are logged the same way (<code>callTool error … err=...</code>, <code>listTools error …</code>, etc.).</li>
</ul>
<h3 id="defaults-cleanup"><a class="anchor" href="#defaults-cleanup" aria-label="Anchor link">#</a>Defaults &amp; cleanup</h3>
<p>Log files live under <code>~/.mcporter/daemon/</code> next to the socket/metadata, or under <code>$XDG_STATE_HOME/mcporter/daemon/</code> when <code>XDG_STATE_HOME</code> is set. They’re not rotated automatically yet; delete/rotate them manually if they grow large. Running <code>mcporter daemon stop</code> leaves the log intact so you can inspect it after a crash.</p>
<h2 id="foreground-debugging"><a class="anchor" href="#foreground-debugging" aria-label="Anchor link">#</a>Foreground debugging</h2>
<p>When troubleshooting deeply, run <code>mcporter daemon start --foreground --log</code> in a dedicated terminal. This keeps the daemon attached (so Ctrl+C stops it) and still writes to the configured log file for later inspection.</p><nav class="page-nav" aria-label="Pager"><a class="page-nav-prev" href="shortcuts.html"><small>Previous</small><span>Agent Shortcuts</span></a><a class="page-nav-next" href="cli-generator.html"><small>Next</small><span>CLI Generator Plan</span></a></nav></article>
        <nav class="toc" aria-label="On this page"><h2>On this page</h2><a class="toc-l2" href="#daemon-logging">Daemon logging</a><a class="toc-l3" href="#cli-flags">CLI flags</a><a class="toc-l3" href="#environment-overrides">Environment overrides</a><a class="toc-l3" href="#per-server-opt-in">Per-server opt-in</a><a class="toc-l3" href="#inspecting-logs">Inspecting logs</a><a class="toc-l3" href="#defaults-cleanup">Defaults &amp;amp; cleanup</a><a class="toc-l2" href="#foreground-debugging">Foreground debugging</a></nav>
      </div>
    </main>
  </div>
  <script>
