...
<head>
  ...
  <title>Quickstart — mcporter</title>
  <meta name="description" content="Quickstart — mcporter documentation.">
  <link rel="canonical" href="https://mcporter.sh/quickstart.html">
  <meta property="og:type" content="website">
  <meta property="og:site_name" content="mcporter">
  <meta property="og:title" content="Quickstart — mcporter">
  <meta property="og:description" content="Quickstart — mcporter documentation.">
  <meta property="og:url" content="https://mcporter.sh/quickstart.html">
  <meta property="og:image" content="https://mcporter.sh/social-card.png">
  <meta property="og:image:width" content="1200">
  <meta property="og:image:height" content="630">
  <meta name="twitter:card" content="summary_large_image">
  <meta name="twitter:title" content="Quickstart — mcporter">
  <meta name="twitter:description" content="Quickstart — mcporter documentation.">
    ...
    <main>
      <header class="hero">
        ...
        <div class="hero-meta">
          ...
          <a class="edit" href="https://github.com/openclaw/mcporter/edit/main/docs/quickstart.md" rel="noopener">Edit page</a>
        </div>
      </header>
      <div class="doc-grid">
        <article class="doc"><h1 id="quickstart">Quickstart</h1>
<p>This walkthrough assumes you already have an MCP server configured in Cursor, Claude Code/Desktop, Codex, Windsurf, OpenCode, or VS Code. If not, copy <a href="https://github.com/steipete/mcporter/blob/main/config/mcporter.example.json"><code>config/mcporter.example.json</code></a> into <code>~/.mcporter/mcporter.json</code> and edit it — see <a href="config.html">Configuration</a> for the full schema.</p>
<h2 id="1-list-the-servers-mcporter-sees"><a class="anchor" href="#1-list-the-servers-mcporter-sees" aria-label="Anchor link">#</a>1. List the servers mcporter sees</h2>
<pre><code class="language-bash">npx mcporter list</code></pre>
<p>You get one row per server with auth status, transport type, and tool count. Add <code>--json</code> for machine output, or <code>--verbose</code> to see which config files registered each server.</p>
<h2 id="2-inspect-a-single-server"><a class="anchor" href="#2-inspect-a-single-server" aria-label="Anchor link">#</a>2. Inspect a single server</h2>
<pre><code class="language-bash">npx mcporter list linear</code></pre>
<p>Single-server output reads like a TypeScript header file: dimmed <code>/** … */</code> doc comments above each <code>function name(...)</code> signature, with optional parameters summarised so the screen stays scannable. Add flags to drill in:</p>
<ul>
<li><code>--brief</code> (alias <code>--signatures</code>) — compact signatures only.</li>
<li><code>--all-parameters</code> — show every optional parameter inline.</li>
<li><code>--schema</code> — pretty-print the JSON schema for each tool.</li>
<li><code>--json</code> — machine-readable schema payload.</li>
</ul>
<p><code>mcporter list shadcn.io/api/mcp.getComponents</code> works too — bare URLs (with or without a <code>.tool</code> suffix or scheme) auto-resolve.</p>
<h2 id="3-call-a-tool"><a class="anchor" href="#3-call-a-tool" aria-label="Anchor link">#</a>3. Call a tool</h2>
<pre><code class="language-bash"># Colon-delimited flags (shell-friendly).
...
npx mcporter call docs.fetch -- --raw-string-with-leading-dashes</code></pre>
<p>Pick the output format with <code>--output text|markdown|json|raw</code>. Use <code>--save-images &lt;dir&gt;</code> to persist binary content blocks. See <a href="cli-reference.html">CLI reference</a> for the full flag list.</p>
<h2 id="4-read-mcp-resources"><a class="anchor" href="#4-read-mcp-resources" aria-label="Anchor link">#</a>4. Read MCP resources</h2>
<pre><code class="language-bash">npx mcporter resource docs                          # list resources
npx mcporter resource docs file:///path/to/spec.md  # read a resource</code></pre>
<p>Output formatting is shared with <code>mcporter call</code> (<code>--output</code>, <code>--json</code>, <code>--raw</code>).</p>
<h2 id="5-generate-a-standalone-cli"><a class="anchor" href="#5-generate-a-standalone-cli" aria-label="Anchor link">#</a>5. Generate a standalone CLI</h2>
<p>When you want to share a tool with someone who shouldn&#39;t have to learn <code>mcporter call</code>:</p>
<pre><code class="language-bash">npx mcporter generate-cli linear --bundle dist/linear.js
node dist/linear.js create-comment --issue-id ENG-123 --body &#39;Looks good!&#39;</code></pre>
<p>Add <code>--compile &lt;path&gt;</code> for a Bun-compiled binary, or <code>--include-tools a,b,c</code> to ship a subset. Full details in <a href="cli-generator.html">CLI generator</a>.</p>
<h2 id="6-emit-typed-clients-for-agents"><a class="anchor" href="#6-emit-typed-clients-for-agents" aria-label="Anchor link">#</a>6. Emit typed clients for agents</h2>
<pre><code class="language-bash">npx mcporter emit-ts linear --mode client --out src/linear-client.ts</code></pre>
<p>You get a <code>.d.ts</code> interface and a <code>createServerProxy()</code>-backed factory. Calls return <code>CallResult</code> objects with <code>.text()</code>, <code>.markdown()</code>, <code>.json()</code>, <code>.images()</code>, <code>.content()</code> helpers — see <a href="tool-calling.html">Tool calling</a> for the proxy API and <a href="emit-ts.html">emit-ts</a> for the generator.</p>
<h2 id="what-next"><a class="anchor" href="#what-next" aria-label="Anchor link">#</a>What next</h2>
<ul>
<li><a href="config.html">Configuration</a> — <code>mcporter.json</code> schema, env interpolation, OAuth fields.</li>
<li><a href="adhoc.html">Ad-hoc connections</a> — point at any MCP endpoint without editing config.</li>
<li><a href="agent-skills.html">Agent skills</a> — wiring per-server skills into a coding agent.</li>
</ul><nav class="page-nav" aria-label="Pager"><a class="page-nav-prev" href="install.html"><small>Previous</small><span>Install</span></a><a class="page-nav-next" href="config.html"><small>Next</small><span>CLI Help Menu Snapshot</span></a></nav></article>
        <nav class="toc" aria-label="On this page"><h2>On this page</h2><a class="toc-l2" href="#1-list-the-servers-mcporter-sees">1. List the servers mcporter sees</a><a class="toc-l2" href="#2-inspect-a-single-server">2. Inspect a single server</a><a class="toc-l2" href="#3-call-a-tool">3. Call a tool</a><a class="toc-l2" href="#4-read-mcp-resources">4. Read MCP resources</a><a class="toc-l2" href="#5-generate-a-standalone-cli">5. Generate a standalone CLI</a><a class="toc-l2" href="#6-emit-typed-clients-for-agents">6. Emit typed clients for agents</a><a class="toc-l2" href="#what-next">What next</a></nav>
      </div>
    </main>
  </div>
  <script>
