...
<head>
  ...
  <title>Known Issues — mcporter</title>
  <meta name="description" content="Known Issues — mcporter documentation.">
  <link rel="canonical" href="https://mcporter.sh/known-issues.html">
  <meta property="og:type" content="website">
  <meta property="og:site_name" content="mcporter">
  <meta property="og:title" content="Known Issues — mcporter">
  <meta property="og:description" content="Known Issues — mcporter documentation.">
  <meta property="og:url" content="https://mcporter.sh/known-issues.html">
  <meta property="og:image" content="https://mcporter.sh/social-card.png">
  <meta property="og:image:width" content="1200">
  <meta property="og:image:height" content="630">
  <meta name="twitter:card" content="summary_large_image">
  <meta name="twitter:title" content="Known Issues — mcporter">
  <meta name="twitter:description" content="Known Issues — mcporter documentation.">
    ...
    <main>
      <header class="hero">
        ...
        <div class="hero-meta">
          ...
          <a class="edit" href="https://github.com/openclaw/mcporter/edit/main/docs/known-issues.md" rel="noopener">Edit page</a>
        </div>
      </header>
      <div class="doc-grid">
        <article class="doc"><h1 id="known-issues">Known Issues</h1>
<p>This file tracks limitations that users regularly run into. Most of these require upstream cooperation or larger refactors—feel free to reference this when triaging bugs.</p>
<h2 id="hosted-oauth-servers-supabase-github-mcp-etc"><a class="anchor" href="#hosted-oauth-servers-supabase-github-mcp-etc" aria-label="Anchor link">#</a>Hosted OAuth servers (Supabase, GitHub MCP, etc.)</h2>
<ul>
<li>Supabase’s hosted MCP server rejects the standard <code>mcp:tools</code> scope and only accepts Supabase-specific scopes (<code>projects:read</code>, <code>database:write</code>, ...). Because they do not expose OAuth discovery metadata or scope negotiation, mcporter cannot auto-register or complete the flow. Workarounds:</li>
<li>Use Supabase’s supported clients (Cursor, Windsurf).</li>
<li>Self-host their MCP server and configure PAT headers / custom OAuth.</li>
<li>Ask Supabase to accept the MCP scope or publish their scope list.</li>
<li>GitHub’s MCP endpoint (<code>https://api.githubcopilot.com/mcp/</code>) returns “does not support dynamic client registration” when mcporter attempts to connect. Copilot’s backend expects pre-registered client credentials. Configure <code>oauthClientId</code>/<code>oauthClientSecretEnv</code> only if the provider gives you a usable OAuth app; otherwise use their supported client or token/header workaround.</li>
<li>Some hosted servers reject dynamic client registration before returning any authorization URL. mcporter now fails those flows immediately instead of waiting for a browser callback that cannot arrive. If the provider supports a pre-registered OAuth app, configure <code>oauthClientId</code>, <code>oauthClientSecretEnv</code>, and the required <code>oauthTokenEndpointAuthMethod</code>; otherwise use the provider&#39;s supported client or token/header workaround.</li>
</ul>
<h2 id="output-schemas-missing-buggy-on-many-servers"><a class="anchor" href="#output-schemas-missing-buggy-on-many-servers" aria-label="Anchor link">#</a>Output schemas missing/buggy on many servers</h2>
<ul>
<li>The MCP spec allows servers to omit <code>outputSchema</code>. In practice, many hosted MCPs return empty or inconsistent schemas, so features that rely on return types (TypeScript signatures, generated CLIs, <code>createServerProxy</code> return helpers) may degrade to <code>unknown</code>.</li>
<li>Workarounds: inspect the server’s README / manual docs for output details, or wrap the tool via <code>createServerProxy</code> and handle the raw envelope manually.</li>
<li>Potential improvement: allow user-provided schema overrides (e.g., <code>mcporter config patch</code>, CLI flag to load schema JSON) so we can fill gaps on a per-tool basis.</li>
</ul>
<h2 id="mcp-sdk-1-22-0-inline-stdio-regression"><a class="anchor" href="#mcp-sdk-1-22-0-inline-stdio-regression" aria-label="Anchor link">#</a>MCP SDK 1.22.0 inline-stdio regression</h2>
<ul>
<li>Upgrading <code>@modelcontextprotocol/sdk</code> to 1.22.0 causes <code>mcporter generate-cli --compile</code> (and direct runtime <code>listTools</code>) to fail against inline STDIO servers with <code>MCP error -32603: Cannot read properties of undefined (reading &#39;typeName&#39;)</code>.</li>
<li>Repro: <code>pnpm mcporter generate-cli &quot;node mock-stdio.mjs&quot; --compile /tmp/inline-cli --runtime bun</code> using the inline stdio harness in <code>tests/cli-generate-cli.integration.test.ts</code>.</li>
<li>Status: reproduced locally; pinned the SDK to <code>~1.21.2</code> until upstream ships a fix.</li>
</ul>
<h2 id="next-steps"><a class="anchor" href="#next-steps" aria-label="Anchor link">#</a>Next Steps</h2>
<ul>
<li>Implement true scope negotiation (read discovery metadata, allow <code>--oauth-scope</code>).</li>
<li>Keep lobbying providers for spec-compliant OAuth behavior.</li>
<li>Consider adding schema override hooks or auto-caching schema snapshots per tool.</li>
</ul>
<p>If you run into other recurring pain points, append them here so we can prioritize fixes.</p><nav class="page-nav" aria-label="Pager"><a class="page-nav-prev" href="tmux.html"><small>Previous</small><span>tmux Hang Diagnostics</span></a><a class="page-nav-next" href="supabase-auth-issue.html"><small>Next</small><span>OAuth Notes &amp; Hosted MCP Compatibility</span></a></nav></article>
        <nav class="toc" aria-label="On this page"><h2>On this page</h2><a class="toc-l2" href="#hosted-oauth-servers-supabase-github-mcp-etc">Hosted OAuth servers (Supabase, GitHub MCP, etc.)</a><a class="toc-l2" href="#output-schemas-missing-buggy-on-many-servers">Output schemas missing/buggy on many servers</a><a class="toc-l2" href="#mcp-sdk-1-22-0-inline-stdio-regression">MCP SDK 1.22.0 inline-stdio regression</a><a class="toc-l2" href="#next-steps">Next Steps</a></nav>
      </div>
    </main>
  </div>
  <script>
