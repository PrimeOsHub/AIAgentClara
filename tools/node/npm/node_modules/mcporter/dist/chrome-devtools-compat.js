...
`;
export function applyChromeDevtoolsCompat(env, command, args) {
...
export function shouldApplyChromeDevtoolsCompat(command, args, env = process.env) {
...
function isChromeDevtoolsToken(token) {
    return (token === 'chrome-devtools-mcp' ||
        token.startsWith('chrome-devtools-mcp@') ||
        token.includes('/chrome-devtools-mcp'));
}
export function resolveChromeDevtoolsCompatPatchPath(candidates = defaultChromeDevtoolsPatchCandidates(), fallbackDir = os.tmpdir()) {
...
//# sourceMappingURL=chrome-devtools-compat.js.map