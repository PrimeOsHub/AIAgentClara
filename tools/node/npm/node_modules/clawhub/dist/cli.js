...
import { cmdDeletePackage, cmdDownloadPackage, cmdExplorePackages, cmdGetPackageTrustedPublisher, cmdInspectPackage, cmdPackageModerationStatus, cmdPackageMigrationStatus, cmdPackageReadiness, cmdPackPackage, cmdPublishPackage, cmdReportPackage, cmdTransferPackage, cmdUndeletePackage, cmdVerifyPackage, } from "./cli/commands/packages.js";
import { cmdPublish } from "./cli/commands/publish.js";
import { cmdCreatePublisher } from "./cli/commands/publishers.js";
import { cmdExplore, cmdInstall, cmdList, cmdPin, cmdSearch, cmdUninstall, cmdUnpin, cmdUpdate, } from "./cli/commands/skills.js";
import { cmdStarSkill } from "./cli/commands/star.js";
import { cmdSync } from "./cli/commands/sync.js";
import { cmdTransferAccept, cmdTransferCancel, cmdTransferList, cmdTransferReject, cmdTransferRequest, } from "./cli/commands/transfer.js";
...
const CLAWSCAN_NOTE_HELP = "This note gives ClawScan context for behavior that may otherwise look unusual, such as network access, native host access, or provider-specific credentials.";
const program = new Command()
    .name("clawhub")
    .description(`${styleTitle(`ClawHub CLI ${getCliBuildLabel()}`)}\n${styleEnvBlock("install, update, search, and publish skills plus OpenClaw packages.")}`)
...
function registerCommand(parent, path) {
    return parent.command(path.at(-1) ?? "");
}
function registerCommandGroup(parent, path) {
    return parent.command(path.at(-1) ?? "");
}
...
const packageCmd = registerCommandGroup(program, ["package"]).description("Browse and publish OpenClaw packages");
registerCommand(packageCmd, ["package", "explore"])
    ...
    .option("--limit <n>", "Number of packages to show (max 100)", (value) => Number.parseInt(value, 10), 25)
    .option("--json", "Output JSON")
    .action(async (queryParts, options) => {
    const opts = await resolveGlobalOpts();
...
registerCommand(packageCmd, ["package", "publish"])
    ...
    .option("--changelog <text>", "Changelog text")
    .option("--clawscan-note <text>", CLAWSCAN_NOTE_HELP)
    .option("--manual-override-reason <reason>", "Required for manual publish when trusted publisher config exists")
    .option("--tags <tags>", "Comma-separated tags", "latest")
...
const transfer = registerCommandGroup(program, ["transfer"]).description("Transfer skill ownership");
