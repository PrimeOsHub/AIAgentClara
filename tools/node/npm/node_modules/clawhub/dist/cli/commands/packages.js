...
function appendPackageExploreFilters(url, options) {
    if (options.target)
        url.searchParams.set("target", options.target);
    if (options.os)
        url.searchParams.set("os", options.os);
    if (options.arch)
        url.searchParams.set("arch", options.arch);
    if (options.libc)
        url.searchParams.set("libc", options.libc);
    if (options.requiresBrowser)
        url.searchParams.set("requiresBrowser", "true");
    if (options.requiresDesktop)
        url.searchParams.set("requiresDesktop", "true");
    if (options.requiresNativeDeps)
        url.searchParams.set("requiresNativeDeps", "true");
    if (options.requiresExternalService)
        url.searchParams.set("requiresExternalService", "true");
    if (options.externalService)
        url.searchParams.set("externalService", options.externalService);
    if (options.binary)
        url.searchParams.set("binary", options.binary);
    if (options.osPermission)
        url.searchParams.set("osPermission", options.osPermission);
    if (options.artifactKind)
        url.searchParams.set("artifactKind", options.artifactKind);
    if (options.npmMirror)
        url.searchParams.set("npmMirror", "true");
}
export async function cmdExplorePackages(opts, query, options = {}) {
    ...
    try {
        ...
        if (trimmedQuery) {
            ...
            const result = await apiRequest(registry, { method: "GET", url: url.toString(), token }, ApiV1PackageSearchResponseSchema);
            spinner.stop();
            if (options.json) {
        ...
        const route = options.family === "code-plugin"
        ...
        appendPackageExploreFilters(url, options);
        const result = await apiRequest(registry, { method: "GET", url: url.toString(), token }, ApiV1PackageListResponseSchema);
        spinner.stop();
        if (options.json) {
            console.log(JSON.stringify(result, null, 2));
            return;
...
export async function cmdInspectPackage(opts, packageName, options = {}) {
...
export async function cmdGetPackageTrustedPublisher(opts, packageName, options = {}) {
    const trimmed = normalizePackageNameOrFail(packageName);
    const token = await getOptionalAuthToken();
    const registry = await getRegistry(opts, { cache: true });
...
export async function cmdPackPackage(opts, sourceArg, options = {}) {
    if (!sourceArg?.trim())
        fail("Path required");
    ...
    if (resolvedSource.kind !== "local")
        fail("Path must be a package folder");
    ...
    if (!packageName)
        fail("package.json name required");
    if (!packageVersion)
        fail("package.json version required");
    ...
    try {
        ...
        const contentValidation = validateOpenClawExternalCodePluginPackageContents(packed.parsed.packageJson, packed.parsed.entries.map((entry) => entry.path));
        ...
        spinner?.succeed(`Packed ${packed.parsed.packageName}@${packed.parsed.packageVersion} -> ${packed.path}`);
        if (options.json) {
...
async function createClawPackFromFolder(options) {
    ...
    let npmOutput = [];
    try {
        npmOutput = JSON.parse(result.stdout);
    }
    catch {
...
export async function cmdPublishPackage(opts, sourceArg, options = {}) {
    if (!sourceArg?.trim())
        fail("Path required");
    let plan;
    try {
        ...
        if (plan.payload.family === "code-plugin") {
            const validation = validateOpenClawExternalCodePluginPackageContents(plan.packageJson, plan.filesOnDisk.map((file) => file.relPath));
            if (validation.issues.length > 0) {
                fail(validation.issues.map((issue) => issue.message).join(" "));
            }
        }
        ...
        try {
            ...
            if (plan.clawpackOnDisk) {
                if (spinner)
                    spinner.text = `Uploading ${plan.clawpackOnDisk.relPath}`;
            ...
            if (spinner)
                spinner.text = `Publishing ${plan.payload.name}@${plan.payload.version}`;
            ...
            if (options.json) {
                process.stdout.write(`${JSON.stringify({ ...plan.output, releaseId: result.releaseId }, null, 2)}\n`);
            }
            else {
                spinner?.succeed(`OK. Published ${plan.payload.name}@${plan.payload.version} (${result.releaseId})`);
            }
        }
        catch (error) {
            spinner?.fail(formatError(error));
...
export async function cmdDownloadPackage(opts, packageName, options = {}) {
    const trimmed = normalizePackageNameOrFail(packageName);
    if (options.version && options.tag)
        fail("Use either --version or --tag");
    const token = await getOptionalAuthToken();
...
export async function cmdVerifyPackage(opts, filePath, options = {}) {
    ...
    try {
        ...
        const expectedSha256 = options.sha256?.trim() ||
            (artifactResult?.artifact.kind === "npm-pack" ? artifactResult.artifact.sha256 : undefined);
        const expectedNpmIntegrity = options.npmIntegrity?.trim() || artifactResult?.artifact.npmIntegrity;
...
export async function cmdDeletePackage(opts, nameArg, options = {}, inputAllowed = true) {
    const name = nameArg.trim();
    if (!name)
        fail("Package name required");
    if (!options.yes) {
        if (!isInteractive() || inputAllowed === false)
...
export async function cmdUndeletePackage(opts, nameArg, options = {}, inputAllowed = true) {
    const name = nameArg.trim();
    if (!name)
        fail("Package name required");
    if (!options.yes) {
...
export async function cmdTransferPackage(opts, nameArg, options) {
...
export async function cmdReportPackage(opts, packageName, options = {}) {
    const trimmed = normalizePackageNameOrFail(packageName);
    const reason = options.reason?.trim();
...
export async function cmdPackageModerationStatus(opts, packageName, options = {}) {
    const trimmed = normalizePackageNameOrFail(packageName);
    const token = await requireAuthToken();
    const registry = await getRegistry(opts, { cache: true });
    const result = await apiRequest(registry, {
...
export async function cmdPackageReadiness(opts, packageName, options = {}) {
    const trimmed = normalizePackageNameOrFail(packageName);
    const token = await getOptionalAuthToken();
    const registry = await getRegistry(opts, { cache: true });
    const result = await apiRequest(registry, {
...
export async function cmdPackageMigrationStatus(opts, packageName, options = {}) {
    const trimmed = normalizePackageNameOrFail(packageName);
    const token = await getOptionalAuthToken();
    const registry = await getRegistry(opts, { cache: true });
    const result = await apiRequest(registry, {
...
async function apiRequestPackageDetail(registry, name, token) {
    return await apiRequest(registry, { method: "GET", path: `${ApiRoutes.packages}/${encodeURIComponent(name)}`, token }, ApiV1PackageResponseSchema);
}
async function apiRequestPackageArtifact(registry, name, version, token) {
...
async function apiRequestPackageTrustedPublisher(registry, name, token) {
    return await apiRequest(registry, {
        method: "GET",
...
async function apiRequestPackageVersion(registry, name, version, token) {
    return await apiRequest(registry, {
        method: "GET",
...
async function apiRequestPackageVersions(registry, name, limit, token) {
    const url = registryUrl(`${ApiRoutes.packages}/${encodeURIComponent(name)}/versions`, registry);
    url.searchParams.set("limit", String(limit));
    return await apiRequest(registry, { method: "GET", url: url.toString(), token }, ApiV1PackageVersionListResponseSchema);
}
async function resolvePackageVersion(registry, name, args) {
    ...
    if (args.tag?.trim()) {
        ...
        if (!tagged)
            fail(`Unknown tag "${args.tag.trim()}"`);
        return tagged;
    }
...
function normalizePackageNameOrFail(raw) {
    const trimmed = raw.trim();
    if (!trimmed)
        fail("Package name required");
    return trimmed;
}
function spinnerText(spinner, text) {
    if (spinner)
        spinner.text = text;
}
...
function computeArtifactIdentity(bytes) {
    return {
...
function digestHex(bytes, algorithm) {
    return createHash(algorithm).update(bytes).digest("hex");
}
function digestBase64(bytes, algorithm) {
    return createHash(algorithm).update(bytes).digest("base64");
}
function validateDownloadedArtifact(requestedPackageName, artifactResult, bytes, identity) {
    ...
    if (artifact.kind === "npm-pack") {
        ...
        if (parsed.packageName !== artifactResult.package.name) {
            fail(`ClawPack package name mismatch: expected ${artifactResult.package.name}, got ${parsed.packageName}`);
        }
        if (parsed.packageVersion !== artifactResult.version) {
            fail(`ClawPack package version mismatch: expected ${artifactResult.version}, got ${parsed.packageVersion}`);
        }
        if (requestedPackageName !== artifactResult.package.name) {
            fail(`Resolved package mismatch: expected ${requestedPackageName}, got ${artifactResult.package.name}`);
        }
    }
    if (requestedPackageName !== artifactResult.package.name) {
        fail(`Resolved package mismatch: expected ${requestedPackageName}, got ${artifactResult.package.name}`);
...
function assertDigestMatch(label, expected, actual) {
    if (!expected)
        return;
    if (expected !== actual) {
...
function defaultArtifactFilename(name, version, artifact) {
    if (artifact.kind === "npm-pack" && artifact.npmTarballName)
        return artifact.npmTarballName;
    const safeName = name
        .replace(/^@/, "")
        .replaceAll("/", "-")
...
async function resolveArtifactOutputPath(opts, output, filename) {
    if (!output?.trim())
        return resolve(opts.workdir, filename);
...
async function assertOutputWritable(path, force) {
    const existing = await stat(path).catch(() => null);
    if (existing && !force)
        fail(`Refusing to overwrite ${path}. Use --force.`);
    await mkdir(dirname(path), { recursive: true });
}
function printPackageSummary(detail) {
    if (!detail.package)
        return;
    ...
    if (pkg.latestVersion)
        console.log(`Latest: ${pkg.latestVersion}`);
...
function printVersionSummary(version) {
    ...
    if (version.changelog.trim())
        console.log(`Changelog: ${truncate(version.changelog, 120)}`);
}
function printTrustedPublisher(trustedPublisher) {
...
function printCompatibility(compatibility) {
    if (!compatibility)
        return;
    const entries = formatCompatibilityEntries(compatibility);
    if (entries.length > 0)
        console.log(`Compatibility: ${entries.join(", ")}`);
}
function formatCompatibilityEntries(compatibility) {
    return [
...
function printCapabilities(capabilities) {
    ...
    if (capabilities.pluginKind)
        console.log(`Plugin kind: ${capabilities.pluginKind}`);
    if (capabilities.bundleFormat)
        console.log(`Bundle format: ${capabilities.bundleFormat}`);
    if (capabilities.hostTargets?.length) {
        console.log(`Host targets: ${capabilities.hostTargets.join(", ")}`);
    }
    if (capabilities.channels?.length)
        console.log(`Channels: ${capabilities.channels.join(", ")}`);
    if (capabilities.providers?.length) {
        console.log(`Providers: ${capabilities.providers.join(", ")}`);
    }
    if (capabilities.toolNames?.length)
        console.log(`Tools: ${capabilities.toolNames.join(", ")}`);
...
function printVerification(verification) {
    ...
    if (verification.sourceRepo)
        console.log(`Source Repo: ${verification.sourceRepo}`);
    if (verification.sourceCommit)
        console.log(`Source Commit: ${verification.sourceCommit}`);
    if (verification.sourceTag)
        console.log(`Source Ref: ${verification.sourceTag}`);
    if (verification.scanStatus)
        console.log(`Scan: ${verification.scanStatus}`);
}
function printArtifact(artifact) {
    ...
    if (summary.sha256)
        console.log(`Artifact SHA-256: ${summary.sha256}`);
    if (typeof summary.size === "number") {
        console.log(`Artifact Size: ${formatByteCount(summary.size)}`);
    }
    if (summary.npmIntegrity)
        console.log(`npm integrity: ${summary.npmIntegrity}`);
    if (summary.npmShasum)
        console.log(`npm shasum: ${summary.npmShasum}`);
    if (summary.npmTarballName)
        console.log(`npm tarball: ${summary.npmTarballName}`);
}
...
function extractReadmeH1(content) {
...
function readReadmeH1FromPackageFiles(files) {
...
function packageJsonString(value, key) {
    const candidate = value?.[key];
    return typeof candidate === "string" && candidate.trim() ? candidate.trim() : undefined;
}
function assertClawPackSize(size, label) {
...
function hasRealBundleMarker(fileSet) {
    return (REAL_BUNDLE_MANIFESTS.some((marker) => fileSet.has(marker.path)) ||
        Array.from(fileSet).some((path) => path.startsWith("skills/") ||
...
function detectPackageFamily(fileSet, explicit) {
    if (explicit)
        return explicit;
    if (hasRealBundleMarker(fileSet))
        return "bundle-plugin";
    if (fileSet.has("openclaw.plugin.json"))
        return "code-plugin";
    return fail("Could not detect package family. Use --family.");
}
async function readBundleManifestInfo(filesOnDisk, folder, parsedClawpack) {
    for (const marker of REAL_BUNDLE_MANIFESTS) {
        const manifest = readJsonEntry(filesOnDisk, marker.path) ??
            (parsedClawpack ? null : await readJsonFile(join(folder, marker.path)));
        if (manifest)
            return { manifest, format: marker.format };
    }
...
function applyGitHubSourcePath(source, sourcePath) {
    const explicitPath = sourcePath?.trim();
    if (!explicitPath || source.kind !== "github")
        return source;
    return { ...source, path: explicitPath };
}
async function preparePackagePublishPlan(opts, sourceArg, options) {
    const resolvedSource = await resolveSourceInput(sourceArg, {
        workdir: opts.workdir,
        localWorkdirs: [process.cwd(), opts.workdir],
    });
    ...
    if (sourceForFetch.kind === "github") {
        ...
        if (folderStat.isFile()) {
            if (!folder.endsWith(".tgz"))
                fail("ClawPack publish files must end in .tgz");
    ...
    const packageJson = parsedClawpack?.packageJson ?? (await readJsonFile(join(folder, "package.json")));
    const pluginManifest = readJsonEntry(filesOnDisk, "openclaw.plugin.json") ??
    ...
    if (family === "code-plugin") {
        if (!fileSet.has("package.json"))
            fail("package.json required");
        if (!source)
            fail("--source-repo and --source-commit required for code plugins");
        const validation = validateOpenClawExternalCodePluginPackageJson(packageJson);
        if (validation.issues.length > 0) {
            fail(validation.issues.map((issue) => issue.message).join(" "));
        }
    ...
    if (family === "code-plugin" && !clawpackOnDisk) {
        ...
        try {
            ...
            if (packed.parsed.packageVersion !== version) {
                fail(`ClawPack package version mismatch: expected ${version}, got ${packed.parsed.packageVersion}`);
            }
        }
        catch (error) {
    ...
    return {
        ...
        packageJson,
        payload,
        compatibility: family === "code-plugin"
...
function readJsonEntry(files, path) {
...
async function requestGitHubActionsOidcToken(audience, options = {}) {
...
async function mintPackagePublishToken(registry, packageName, version, githubOidcToken) {
...
async function resolvePackagePublishToken(params) {
    ...
    try {
        ...
        return await mintPackagePublishToken(params.registry, params.packageName, params.version, githubOidcToken);
    }
    catch (error) {
...
function buildSource(options, inferred) {
    ...
    if (!rawRepo && !rawCommit && !rawRef && !rawPath)
        return undefined;
    if (!rawRepo || !rawCommit)
        fail("--source-repo and --source-commit must be set together");
    const repo = normalizeGitHubRepo(rawRepo);
    if (!repo)
        fail("--source-repo must be a GitHub repo or URL");
...
function describePublishSource(sourceInput, source, folder) {
    if (source) {
    ...
    if (sourceInput.kind === "github") {
        ...
        return `github:${repo}@${sourceInput.ref ?? "HEAD"}${sourceInput.path !== "." ? `:${sourceInput.path}` : ""}`;
    }
    return `local:${folder}`;
}
function printPackageDryRun(params) {
    ...
    console.log(`Files:     ${params.files.length} files (${formatByteCount(params.files.reduce((sum, file) => sum + file.bytes.byteLength, 0))})`);
