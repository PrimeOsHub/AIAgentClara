...
import { ApiRoutes, ApiV1SearchResponseSchema, ApiV1SkillListResponseSchema, ApiV1SkillReportListResponseSchema, ApiV1SkillReportResponseSchema, ApiV1SkillReportTriageResponseSchema, ApiV1SkillResolveResponseSchema, ApiV1SkillResponseSchema, ApiV1SkillVersionResponseSchema, } from "../../schema/index.js";
import { extractZipToDir, hashSkillFiles, listManualSkills, listTextFiles, readLockfile, readSkillOrigin, writeLockfile, writeSkillOrigin, } from "../../skills.js";
...
function isPinnedSkillEntry(entry) {
    return entry?.pinned === true;
}
function withPinnedMetadata(version, installedAt, existing) {
    return {
        version,
        installedAt,
...
function formatPinnedDetails(entry) {
...
export async function cmdSearch(opts, query, limit) {
    ...
    try {
        ...
        for (const entry of result.results) {
            ...
            console.log(`${slug}${version}  ${formatSearchOwner(entry)}  ${name}  (${entry.score.toFixed(3)})`);
        }
    }
    catch (error) {
    ...
    try {
        ...
        if (versionFlag) {
            await apiRequest(registry, {
                method: "GET",
                path: `${ApiRoutes.skills}/${encodeURIComponent(trimmed)}/versions/${encodeURIComponent(resolvedVersion)}`,
                token,
        ...
        const installedFingerprint = installedFiles.length > 0 ? hashSkillFiles(installedFiles).fingerprint : undefined;
...
export async function cmdUpdate(opts, slugArg, options, inputAllowed) {
    ...
    if (slugs.length === 0) {
        if (skippedPinned.length > 0) {
            ...
            console.log(`Skipped ${skippedPinned.length} pinned skill${suffix}: ${skippedPinned.join(", ")}`);
            return;
        }
    ...
    for (const entry of slugs) {
        ...
        try {
            ...
            const matched = resolveResult.match?.version ??
                (localFingerprint &&
            ...
            if (matched && lock.skills[entry]?.version !== matched) {
                lock.skills[entry] = withPinnedMetadata(matched, lock.skills[entry]?.installedAt ?? Date.now(), lock.skills[entry]);
            }
            if (!latest) {
...
export async function cmdPin(opts, slug, options = {}) {
    const trimmed = normalizeSkillSlugOrFail(slug);
    const lock = await readLockfile(opts.workdir);
...
export async function cmdUnpin(opts, slug) {
    const trimmed = normalizeSkillSlugOrFail(slug);
    const lock = await readLockfile(opts.workdir);
    const existing = lock.skills[trimmed];
    if (!existing)
        fail(`Not installed: ${trimmed}`);
    if (!isPinnedSkillEntry(existing))
        fail(`Skill "${trimmed}" is not pinned`);
    lock.skills[trimmed] = {
...
export async function cmdReportSkill(opts, slug, options = {}) {
    const trimmed = normalizeSkillSlugOrFail(slug);
...
export async function cmdListSkillReports(opts, options = {}) {
    ...
    const result = await apiRequest(registry, { method: "GET", url: url.toString(), token }, ApiV1SkillReportListResponseSchema);
    if (options.json) {
        process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
        return;
    }
    if (result.items.length === 0) {
        ...
        for (const item of result.items) {
            ...
            if (item.triageNote)
                console.log(`  note: ${item.triageNote}`);
        }
    }
    if (!result.done && result.nextCursor)
        console.log(`Next cursor: ${result.nextCursor}`);
}
export async function cmdTriageSkillReport(opts, reportId, options = {}) {
    ...
    const finalAction = (options.finalAction ?? options.action)?.trim();
    if (finalAction && !["none", "hide"].includes(finalAction)) {
        fail("--action must be none or hide");
    }
    const note = options.note?.trim();
    if (status !== "open" && !note)
        fail("--note required unless reopening");
    const token = await requireAuthToken();
    const registry = await getRegistry(opts, { cache: true });
    await presentModerationPlan(reportModerationPlan({
        entityLabel: "skill",
        reportId: trimmed,
        status,
    ...
    const actionSuffix = result.actionTaken && result.actionTaken !== "none" ? `; action ${result.actionTaken}` : "";
...
function resolveExploreSort(raw) {
    ...
    return fail(`Invalid sort "${raw}". Use newest, updated, downloads, rating, installs, installsAllTime, or trending.`);
}
