...
function normalizeSlug(slugArg, label = "Skill slug") {
...
export async function cmdRenameSkill(opts, slugArg, newSlugArg, options, inputAllowed) {
    ...
    const confirmed = await requireYesOrConfirm(options, inputAllowed, `Rename ${slug} to ${newSlug}? Old slug will redirect.`);
    if (!confirmed)
        return undefined;
...
export async function cmdMergeSkill(opts, sourceSlugArg, targetSlugArg, options, inputAllowed) {
    ...
    const confirmed = await requireYesOrConfirm(options, inputAllowed, `Merge ${sourceSlug} into ${targetSlug}? Source slug will redirect and stop listing publicly.`);
    if (!confirmed)
        return undefined;
    const token = await requireAuthToken();
    const registry = await getRegistry(opts, { cache: true });
