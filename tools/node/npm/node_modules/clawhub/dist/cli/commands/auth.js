...
async function cmdLogin(opts, tokenFlag, inputAllowed) {
 ...
 */
export async function cmdDeviceLogin(opts) {
    const discovery = await discoverRegistryFromSite(opts.site).catch(() => null);
    const authBase = discovery?.authBase?.trim() || opts.site;
    const registry = await getRegistry(opts, { cache: true });
    ...
    try {
        const tokenResponse = await pollForDeviceToken({ apiUrl: registry, siteUrl: authBase }, deviceCode.device_code, { interval: deviceCode.interval, expiresIn: deviceCode.expires_in });
