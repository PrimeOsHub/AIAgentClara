...
export async function checkRegistrySyncState(registry, skill, resolveSupport, token) {
    ...
    const meta = await apiRequest(registry, { method: "GET", path: `${ApiRoutes.skills}/${encodeURIComponent(skill.slug)}`, token }, ApiV1SkillResponseSchema).catch(() => null);
...
export async function selectToUpload(candidates, params) {
    ...
    if (isCancel(picked))
        fail("Canceled");
    const selected = picked.map((key) => valueByKey.get(key)).filter(Boolean);
    return selected;
}
