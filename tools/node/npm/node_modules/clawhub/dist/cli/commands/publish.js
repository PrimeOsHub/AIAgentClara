...
import { ApiRoutes, ApiV1PublishResponseSchema, normalizeClawScanNote, } from "../../schema/index.js";
...
export async function cmdPublish(opts, folderArg, options) {
    ...
    try {
        const filesOnDisk = stripGeneratedSkillCards(await ensureRootManifestFile(folder, await listTextFiles(folder)));
...
function stripGeneratedSkillCards(files) {
    return files.filter((file) => file.relPath.trim().toLowerCase() !== "skill-card.md");
}
async function ensureRootManifestFile(folder, files) {
    if (files.some((file) => {
        const lower = file.relPath.toLowerCase();
        return lower === "skill.md" || lower === "skills.md";
    })) {
...
async function looksLikePluginFolder(folder) {
    const checks = [
    ...
    try {
        const raw = JSON.parse(await readFile(checks[1], "utf8"));
